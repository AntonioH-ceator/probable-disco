...
<body>
    <?php 
        ...
        $search = $_GET["search"] ?? "";
        if (!$search){
            header("location: ../ATech_3");
        }
        $query = clean($search);
        $cmd = $pdo->query("select *, MATCH (nome ,descricao)  AGAINST ('$search' with query expansion)
        AS score FROM cursos where nome like '%$search%' or descricao like '%$search%' ORDER BY score;");
        ...
        <div class="main_container flex-space-between">
            ...
            <div class="center-content search-bar-comp">
                <form action="search.php" method="get">
                    <input type="text" name="search" class="search-bar" placeholder="Procurar..." <?php
                        $search = $_GET["search"];
                        echo("value='$search'");
                    ?>>
                </form>
    ...
    <div class="main_container left-align">
        ...
        <?php
            if (!$qtd_result){
                echo("<h2>Nenhum documento corresponde com a pesquisa '$search'.</h2><p>Sugestões:</p><ul style='margin-left:1.3em;margin-bottom:2em'><li>Certifique-se de que todas as palavras estejam escritas corretamente.</li><li>Tente palavras-chave diferentes.</li><li>Tente palavras-chave mais genéricas.</li></ul>");
            }else{
                echo("<h2>Resultados para '$search': <br><div class='description'>Resultados encontrados: $qtd_result</div></h2>");
            }
